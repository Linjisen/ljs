...
static void  wlan_mgnt_sta_password_wrong_event(struct rt_wlan_device *device, rt_wlan_event_t event, void *user_data)
{
     WLAN_MGNT_DBG("wlan sta password wrong event callback \n");
}
...
int rt_wlan_mgnt_attach(struct rt_wlan_device *device, void *user_data)
{
    ...
    rt_wlan_register_event_handler(device, WIFI_EVT_STA_PASSWORD_WRONG, wlan_mgnt_sta_password_wrong_event);
