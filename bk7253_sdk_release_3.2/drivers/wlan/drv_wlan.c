...
extern void bk_ap_no_password_connected_register_cb(bk_ap_no_password_cb_t func);
...
static void wlan_event_handle(void *ctx)
{
    ...
    case RW_EVT_STA_PASSWORD_WRONG:
        rt_kprintf("rl_pre_sta_get_status:%d\r\n",rl_pre_sta_get_status());
        if(RL_STATUS_STA_CONNECTING==rl_pre_sta_get_status() || RL_STATUS_STA_LAUNCHED==rl_pre_sta_get_status())
        {
            rt_wlan_indicate_event_handle(&_g_sta_device, WIFI_EVT_STA_PASSWORD_WRONG, RT_NULL);
        }
        break;
