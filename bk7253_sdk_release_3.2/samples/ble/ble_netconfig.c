...
extern void gapm_set_max_mtu(uint16_t mtu);
...
static ble_err_t ble_create_db(void)
{
    ...
    ble_db_cfg.att_db = (bk_attm_desc_t*)netconfig_att_db;
...
static void ble_write_callback(write_req_t *param)
{
    ...
    else if(ble_session->status == RECVING)
    {
        ...
        if((rt_tick_get() - ble_session->tick) > BLE_TIMEOUT)
        {
            ...
            ble_clean_data();
            if(str_begin_with((const char *)param->value, "{") != RT_NULL)
            {
                ble_push_data(param->value, param->len);
                ble_session->tick = rt_tick_get();
    ...
    if(str_end_with((const char *)param->value, param->len, "}"))
    {
...
static void bk_ble_netconfig_thread(void *parameter)
{
...
_exit:
    tid=NULL;
    ble_netconfig_stop();
    rt_kprintf("bk_ble_netconfig_thread exit\r\n");
    return ;
}
...
{
    rt_kprintf("bk_ble_netconfig_start\r\n");
    if (tid)
    {
