...
static void i2c1_isr(void)
{
    ...
    switch (work_mode)
    {
        case 0x00:      // master write
        {
            ...
            {
                REG_WRITE(REG_I2C1_DAT, gi2c1.RegAddr);
                gi2c1.AddrFlag |= 0x13;
            }
        }
        break;
...
static UINT32 i2c1_write(char *user_buf, UINT32 count, UINT32 op_flag)
{
    ...
    reg |= I2C1_STA;// Set STA
    REG_WRITE(REG_I2C1_CONFIG, reg);
    i2c1_write_time=rt_tick_get();
    while(!gi2c1.TransDone)
    {
        if(rt_tick_get()-i2c1_write_time>I2C1_WAIT_TIMEOUT)
        {
            //printf("++++++++++++++++++++i2c write timeout gi2c1.RemainNum:%d\r\n",gi2c1.RemainNum);
            gi2c1.RemainNum=count;
            break;
        }
    }
