...
static int IR_send_key(UINT32 msg)
{
	if(NULL != IR_key.IRkey_mq)
		return rt_mq_send(IR_key.IRkey_mq, (void *)&msg, sizeof(msg));
	else
		return -1;
}
...
#define STOP_TIMER (1 << 1)
	...
	UINT8	op_flag = 0;
	IR_key.timer_cnt++;
//	rt_kprintf("---timer handle:%x,%x---\r\n",IR_key.timer_cnt,IR_key.repeat_cnt);
...
rt_err_t IR_get_key(void *buffer, rt_size_t  size, rt_int32_t timeout)
{
