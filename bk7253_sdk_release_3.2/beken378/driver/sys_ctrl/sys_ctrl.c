...
static RESET_SOURCE_STATUS sctrl_set_wake_soure_status(void);
...
void sctrl_mcu_exit(void)
{
    ...
    reg &= ~HCLK_DIV2_EN_BIT;
#endif
    reg |= ((MCLK_DIV_3 & MCLK_DIV_MASK) << MCLK_DIV_POSI); //120M
...
void sctrl_unconditional_normal_wakeup()
{
    ...
    sctrl_unconditional_mac_exit_doze();
    delay(10);

    sctrl_ctrl(CMD_SCTRL_OPEN_ADC_MIC_ANALOG, NULL);
    sctrl_ctrl(CMD_SCTRL_OPEN_DAC_ANALOG, NULL);
