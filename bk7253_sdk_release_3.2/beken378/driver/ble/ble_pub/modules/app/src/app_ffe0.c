                                     ...
                                     kernel_task_id_t const src_id)
{
    ...
    write_req.prf_id = TASK_ID_FFE0S;
                                     ...
                                     kernel_task_id_t const src_id)
{
    ...
    uint16_t status=ffe0s_get_att_idx(param->handle, &read_req.att_idx);
    //read_req.att_idx = param->handle - ble_env->start_hdl;
    read_req.prf_id = TASK_ID_FFE0S;
    ...
    struct ffe0s_ffe1_read_rsp * rsp = KERNEL_MSG_ALLOC_DYN(FFE0S_FFE1_READ_RSP,
                                                        prf_get_task_from_id(TASK_ID_FFE0S),
                                                        TASK_APP,
                                                        ffe0s_ffe1_read_rsp,length);

    rsp->length = length;
    rsp->handle = param->handle;
    rsp->rsp_id = param->rsp_id;
    memcpy(rsp->value,read_req.value,rsp->length);
