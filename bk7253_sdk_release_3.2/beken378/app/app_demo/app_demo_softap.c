...
static void app_demo_softap_rw_disconnected_event_func(void *ctx)
{
    rt_kprintf("into init\r\n");
	rw_evt_type evt_type = *((rw_evt_type*)ctx);
	rt_kprintf("evt_type:%d\r\n",evt_type);
...
static void app_demo_softap_main( beken_thread_arg_t data )
{
 ...
 //   rw_evt_set_callback(RW_EVT_AP_DISCONNECTED, app_demo_softap_rw_event_func);
	bk_wlan_status_register_cb(app_demo_softap_rw_disconnected_event_func);
