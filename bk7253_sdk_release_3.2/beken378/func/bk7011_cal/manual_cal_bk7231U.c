...
UINT32 manual_cal_check_pwr_idx(UINT32 *level)
{
    UINT8 ch1 = GET_TXPWR_GAIN(&gtxpwr_tab_g[0]);
    UINT8 ch7 = GET_TXPWR_GAIN(&gtxpwr_tab_g[6]);
    UINT8 ch13 = GET_TXPWR_GAIN(&gtxpwr_tab_g[12]);
    UINT8 adv;
    UINT32 ret = 0;
...
{	
    ...
    {		
        ...
        if((GET_TXPWR_FLAG(&tab_ptr[0]) == TXPWR_ELEM_INUSED)	
            ...
            SET_TXPWR_FLAG(&gtxpwr_tab_ble[0],TXPWR_ELEM_UNUSED);
            SET_TXPWR_FLAG(&gtxpwr_tab_ble[39],TXPWR_ELEM_UNUSED);
        }	
    }
}
