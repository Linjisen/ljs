...
void mhdr_disconnect_ind(void *msg)
{
	...
	os_printf("%s reason_code=%d,state:%d\n", __FUNCTION__, disc->reason_code,rl_pre_sta_get_status());
#if CFG_ROLE_LAUNCH
//when disconnect, don't connect ap automatic 

	if(get_bk_wlan_stop_state())
		return;
