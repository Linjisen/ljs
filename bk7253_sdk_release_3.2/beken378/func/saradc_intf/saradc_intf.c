...
static void tadc_thread_entry(void *pv)
{
    ...
    while(1)
    {
    	rtos_delay_milliseconds(scan_tick);
		rtos_lock_mutex(&tadc_entity->obj_list_mutex);
...
void saradc_work_create(UINT32 scan_interval_ms)
{
	...
	result = rtos_create_thread(&tadc_entity->tadc_thread, ADC_TASK_PRIORITY, ADC_TAG, \
						tadc_thread_entry, ADC_TASK_STACK_SIZE, (beken_thread_arg_t) scan_interval_ms);
	if(kNoErr == result)
	{
		return;
	}
