...
int wpa_psk_request(u8 *ssid, size_t ssid_len, char *passphrase, u8 *psk, size_t psk_len)
{
	...
	os_memset(cache->item.psk, 0, sizeof(cache->item.psk));
...
void start_wpa_psk_cal_thread()
{
	...
	if (!wpa_pskcalc_thread_handle) {
		rtos_create_thread(&wpa_pskcalc_thread_handle, THD_PSK_PRIORITY,	/* lower or equal than wpas thread */
